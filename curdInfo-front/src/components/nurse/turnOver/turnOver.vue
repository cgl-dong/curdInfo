<template>
  ...
  <el-dialog title="添加记录人员流动信息" :visible.sync="dialogFormVisible">
        <el-form class="small-space" :model="form" label-position="left" label-width="70px" style='width: 400px; margin-left:50px;'>
          ...
          <el-form-item label="工龄">
            <el-input v-model="form.seniority" placeholder="0年"></el-input>
          </el-form-item>
          <el-form-item label="员工编号">
            <el-input v-model="form.number"  placeholder="0000号"></el-input>
          </el-form-item>
          <el-form-item label="职称">
...
<!--            <el-input v-model="form.hiredate"></el-input>-->
             <el-date-picker type="date" placeholder="选择日期" v-model="form.hiredate" value-format="yyyy-MM-dd">
             </el-date-picker>
          </el-form-item>
           <el-form-item label="离职时间">
<!--            <el-input v-model="form.resignation_time"></el-input>-->
             <el-date-picker type="date" placeholder="选择日期" v-model="form.resignation_time" value-format="yyyy-MM-dd">
             </el-date-picker>
          </el-form-item>
           <el-form-item label="离职原因">
  ...
  export default {
    ...
    methods: {
      ...
      handleCreateSubmit () {
        ...
        //   type: 'success'
        // });
        this.$axios.post('his/turnover/nurse', {
          name: this.form.name,
          age: this.form.age,
          sex: this.form.sex,
          ...
          hiredate: this.timeFormat_h(),
          resignation_time: this.timeFormat_r(),
          resignation_reason: this.form.resignation_reason
        }).then(() => {
          this.$message({
            message: '添加成功',
            type: 'success'
      ...
      timeFormat_h () {
        let oldDate = this.form.hiredate;
      ...
      timeFormat_r () {
        let oldDate = this.form.resignation_time;
        let year = new Date(oldDate).getFullYear();
        let month = new Date(oldDate).getMonth() + 1;
        let day = new Date(oldDate).getDate();
        let dateFormat = year + '/' + month + '/' + day;
    ...
    created () {
      this.$axios.get('his/turnover/list').then((response) => {             // mark
