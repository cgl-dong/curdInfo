...
<!--          </el-form-item>-->
      ...
      <el-dialog title="修改值班信息" :visible.sync="dialogFormEditVisible">
        <el-form class="small-space" :model="form" label-position="left" label-width="70px" style='width: 400px; margin-left:50px;'>
          <el-form-item label="姓名">
            <el-input v-model="tableEdit.name" placeholder="必填" disabled></el-input>
          </el-form-item>
          <el-form-item label="日期">
...
<script type="text/ecmascript-6">
  export default {
    ...
    methods: {
      ...
      // 添加提交
      handleCreateSubmit () {
        this.$axios.post('his/nurse/table/shift', {
          name: this.form.name,
          date: this.timeFormat(),
          time: this.form.time,
          phone: this.form.phone,
          attendence: this.form.attendence,
    ...
    created () {
      this.$axios.get('his/nurse/table').then((response) => {             // mark
        this.table = response.data;
        this.searchList = response.data;
        console.log(this.table[0].name);
      }, response => {
        // error callback
        this.$message({
          message: '数据请求失败',
