<template>
    <div class="nurse_table">
      ...
      @current-change="handleCurrentChange"  :page-sizes="pSizes" :page-size="pSize" layout="total, sizes, prev, pager, next, jumper" :total="allNurse">
         </el-pagination>
      </div>
      <!-- 添加信息功能的表单 -->
      ...
      <el-dialog title="修改个人信息" :visible.sync="dialogFormEditVisible">
        <el-form class="small-space" :model="form" label-position="left" label-width="70px" style='width: 400px; margin-left:50px;'>
          <el-form-item label="姓名">
            <el-input v-model="tableEdit.name" disabled></el-input>
          </el-form-item>
          <el-form-item label="地址">
...
export default {
  ...
  methods: {
    ...
    handleEditSubmit (index) {
      ...
      this.$axios.post('his/nurse/edit', {
        name: this.tableEdit.name,
        address: this.tableEdit.address,
        age: this.tableEdit.age,
        position: this.tableEdit.position,
        profession: this.tableEdit.profession,
        department: this.tableEdit.department,
        education: this.tableEdit.education,
        sex: this.tableEdit.sex,
        date: this.tableEdit.date
      }).then(res => {
    ...
    // 搜索
    handelSearch () {
      console.log(this.searchName + this.searchDepartment);
      let intendedSearch = this;
      if (this.searchName === '' && this.searchDepartment !== '') {
        this.$axios.get('/his/nurse/search/department?department=' + this.searchDepartment)
          .then(response => {
            intendedSearch.table = [];
            for (let i = 0; i < response.data.length; i++) {
              if (response.data[i].name === intendedSearch.searchName && response.data[i].department === intendedSearch.searchDepartment) {
                intendedSearch.table.push(response.data[i]);
              } else if (response.data[i].name === intendedSearch.searchName) {
                intendedSearch.table.push(response.data[i]);
              } else if (response.data[i].department === intendedSearch.searchDepartment) {
                intendedSearch.table.push(response.data[i]);
              }
            }
          });
      } else if (this.searchDepartment === '' && this.searchName !== '') {
        this.$axios.get('/his/nurse/search/name?name=' + this.searchName)
          .then(response => {
            intendedSearch.table = [];
            for (let i = 0; i < response.data.length; i++) {
              intendedSearch.table.push(response.data[i]);
        ...
        this.$axios.get('/his/nurse/search?name=' + this.searchName + '&department=' + this.searchDepartment)
          .then(response => {
            intendedSearch.table = [];
            for (let i = 0; i < response.data.length; i++) {
              if (response.data[i].name === intendedSearch.searchName && response.data[i].department === intendedSearch.searchDepartment) {
    ...
    handleSizeChange (val) {
      this.$axios.get('his/nurse/hasNurse/page?offset=1&limit=' + val).then((response) => {             // mark
        this.table = response.data.records;
        this.searchList = response.data.records;
        this.allNurse = response.data.total;
        console.log('pageSize' + this.pSize);
      }, response => {
        // error callback
        this.$message({
          message: '数据请求失败',
    ...
    handleCurrentChange (val) {
      this.$axios.get('his/nurse/hasNurse/page?offset=' + val + '&limit=' + this.pSize).then((response) => {             // mark
        this.table = response.data.records;
        this.searchList = response.data.records;
        this.allNurse = response.data.total;
      }, response => {
  ...
  created () {
    this.$axios.get('his/nurse/hasNurse/page?offset=1&limit=' + this.pSize).then((response) => {             // mark
      this.table = response.data.records;
      this.searchList = response.data.records;
      this.allNurse = response.data.total;
    }, response => {
