  ...
  <!-- Tabs 标签页 -->
    <el-tabs v-model="activeName">
      <el-tab-pane label="患者留言" name="first">
        <template>
        <!-- 表格 -->
          <el-table :data="complaintPatientList" height="800" border style="width: auto">
            ...
            <el-table-column prop="content" label="投诉内容" width="500"></el-table-column>
      ...
      <el-tab-pane label="投诉处理" name="second">
        <div class="importants">
          <h2>一、投诉途径与渠道</h2>
            <ol>
              <li v-for="item in complaintList">{{item.way}}</li>
            </ol>
          <h2>二、受理投诉的部门和范围</h2>
            <ol>
              <li v-for="item in complaintList">{{item.department}}</li>
            </ol>
        </div>
      </el-tab-pane>
...
<script type="text/ecmascript-6">
    export default {
      ...
      mounted () {
        this.$axios.get('his/department/complain').then((response) => {             // mark
          this.complaintList = response.data;
          this.complaintPatientList = response.data;
        }, response => {
          // error callback
          alert('1数据请求失败');
        });
        this.$axios.get('his/patient/list').then((response) => {             // mark
