...
<script type="text/ecmascript-6">
  import patList from '../../../global/patient_drug';
  export default {
    data () {
      return {
        prescriptionC: [],
    ...
    mounted () {
      ...
      prescriptionCThis.prescriptionC = patList.tableData;
      // prescriptionCThis.$http.get('/static/patientList.json').then((response) => {
      //   console.log(api.patientList);
    ...
    methods: {
      ...
      search () {
        ...
        for (let i = 0; i < patList.tableData.length; i++) {
          // 两个条件都有输入值的时候，判断是否有同时符合这两个条件的数据
          if (search.searchNumber && search.searchName) {
            if (patList.tableData[i].number === search.searchNumber && patList.tableData[i].name === search.searchName) {
              search.prescriptionC.push(patList.tableData[i]);
            }
          } else {
            if (search.searchNumber && patList.tableData[i].number === search.searchNumber) {
              // 只有第一个输入值的时候
              search.prescriptionC.push(patList.tableData[i]);
            } else if (search.searchName && patList.tableData[i].name === search.searchName) {
              // 只有第二个输入值的时候
              search.prescriptionC.push(patList.tableData[i]);
            }
          }
