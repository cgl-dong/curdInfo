<template>
  <div class="drugApplyC">
    ...
    <h4 class="tips">以下药品库存不足20件了，建议申请补给！</h4>
  ...
  export default {
    ...
    methods: {
      ...
      getinsufficientData: function () {
        ...
        drugapplyThis.$axios.get('his/drug/c').then((response) => {
          // 把json接口获取的数据
          for (let i = 0; i < response.data.length; i++) {
            if (response.data[i].quantity < 20) {
              drugapplyThis.insufficientTable.push(response.data[i]);
            }
          }
        }, response => {
          // error callback
