    ...
    <el-form :model="register" :rules="rules" ref="registerForm" label-width="100px" class="register-ruleForm">
      ...
      <el-form-item label="科室" prop="department_value">
        <el-select v-model="departmentvalue" placeholder="请选择科室">
          <el-option v-for="items in register.department" :key="items.value" :label="items.text " :value="items.text">
          </el-option>
        </el-select>
      </el-form-item>
...
<script type="text/ecmascript-6">
  import depart from '../../../global/department';
  export default {
    data () {
      // 验证输入身份证号格式是否正确的方法
    ...
    created () {
      ...
      this.register.department = depart.department;
    },
    methods: {
      submitForm (formName) {
        this.$refs.registerForm.validate((valid) => {
          if (valid) {
            this.$axios.post('his/register/add', {
              no: this.no,
              number: '202009' + this.no,
              patientName: this.register.patientName,
              certificateValue: this.register.certificate_value,
              cardNumber: this.register.cardNumber,
              departmentValue: this.register.department_value,
              doctorName: this.register.doctor_value,
              visitDate: this.visit_date + ' ' + this.visit_time,
              expense: this.register.expense + '元'
            }).then(res => {
              this.no += 1;
              this.$notify({
                message: '提交成功',
    ...
    watch: {
      departmentvalue: function () {
        ...
        console.log(this.departmentvalue); // mark: 为什么监听的属性只能是一个字符串???
        let registerThis = this;
        this.register.doctor = [];
        registerThis.register.department_value = registerThis.departmentvalue;
        registerThis.$axios.post('his/register/search?office=' + this.departmentvalue).then((response) => {
          // 把json接口获取的数据赋给当前对象
          console.log(response.data);
          for (let i = 0; i < response.data.length; i++) {
            if (registerThis.departmentvalue === response.data[i].office) {
              registerThis.register.doctor.push(response.data[i]);
              // mark ：此处留下扩展点，专家号和普通号
              if (response.data[i].isexpert === 'yes') {
                registerThis.register.expense = 15;
              } else if (response.data[i].isexpert === 'no') {
                registerThis.register.expense = 10;
              }
            } else {
